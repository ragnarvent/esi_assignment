...
public interface PlantInvEntryRepository extends JpaRepository<PlantInvEntry, Long>, QueryDslPredicateExecutor<PlantInvEntry> {
	...
	@Query("select distinct p.plantInfo from PlantInvItem p where LOWER(p.plantInfo.name) like LOWER(CONCAT('%', ?1, '%')) and p.id not in (select t.plantItemId from PlantReservation t where "
