...
public interface PlantInvItemRepository extends JpaRepository<PlantInvItem, Long>, QueryDslPredicateExecutor<PlantInvItem> {
	...
	@Query("select p from PlantInvItem p where p not in (select t.plantItemId from PlantReservation t where t.rentalPeriod.startDate > ?1 )")
	public List<PlantInvItem> queryUnhiredPlant(LocalDate date);
	
	@Query("select p from PlantInvItem p where p.plantInfo.id = ?1 and p.id not in (select t.plantItemId from PlantReservation t where "
