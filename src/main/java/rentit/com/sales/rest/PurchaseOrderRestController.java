...
public class PurchaseOrderRestController {
	...
	@RequestMapping(method=RequestMethod.POST, path = "/{id}/plant/{status}")
	public PurchaseOrderDTO handlePlantDelivery(
			@PathVariable("id") Long id,
			@PathVariable("status") String status)
				throws PurchaseOrderNotFoundException {
		return salesService.modifyPoPlantState(
			id, POPlantStatus.valueOf(status.toUpperCase())
		);
	}
